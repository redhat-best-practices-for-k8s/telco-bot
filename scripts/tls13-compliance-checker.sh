...
#   - HIGH: MinVersion/MaxVersion set to TLS 1.0 or 1.1 (known vulnerabilities)
#   - MEDIUM: MaxVersion set to TLS 1.2 (prevents TLS 1.3 negotiation)
#   - INFO: MinVersion set to TLS 1.3 (may break older clients)
...
#   3. git must be available for cloning/updating repositories
...
#
# PERFORMANCE:
#   Using local clones instead of GitHub Code Search API provides:
#   - No API rate limits (10 req/min with API)
#   - ~1-2 seconds per repo instead of ~1 minute
#   - Full scan of 200+ repos in 5-10 minutes
#   - Clones persist for future runs
#
# LIMITATIONS:
#   - Limited to 1000 repositories per organization
#   - Excludes vendor/, testdata/, mocks/, and *_test.go files
#===============================================================================
...
show_help() {
	...
	echo "    issues and anti-patterns. Uses local clones for fast, rate-limit-free"
	echo "    scanning. Identifies insecure settings like InsecureSkipVerify,"
	echo "    weak TLS versions, and deprecated options."
	echo
	...
	echo "    - git must be available for cloning/updating repositories"
	echo
...
scan_local_repo() {
	...
	for pattern_def in "${TLS_PATTERNS[@]}"; do
		...
		# Search for pattern, excluding vendor, testdata, mocks, and test files
		local matches=$(grep -rn --include="*.go" -E "$regex" "$repo_path" \
			--exclude-dir=vendor --exclude-dir=testdata --exclude-dir=mocks \
			--exclude-dir=.git --exclude="*_test.go" 2>/dev/null)
		...
		if [ -n "$matches" ]; then
			...
			# Add to findings using jq
			findings=$(echo "$findings" | jq --arg s "$severity" --arg p "$pattern" \
				--arg d "$description" --arg f "$files" --arg c "$count" \
				'. + [{severity: $s, pattern: $p, description: $d, files: $f, count: ($c|tonumber)}]')
		fi
	done
...
update_cache_result() {
	...
	jq --arg repo "$repo" --argjson findings "$findings" --arg timestamp "$timestamp" \
		'.repositories[$repo] = {findings: $findings, last_checked: $timestamp}' "$RESULTS_CACHE" >"$temp_cache" 2>/dev/null && mv "$temp_cache" "$RESULTS_CACHE" || rm -f "$temp_cache"
}
...
echo "Generating markdown report: $OUTPUT_MD"
{
	...
	if [ "$REPOS_WITH_ISSUES" -gt 0 ]; then
		echo "## Findings by Repository"
		echo ""

		# Create temp jq files for reliable query execution
		JQ_MD_COUNT=$(mktemp)
		JQ_MD_TABLE=$(mktemp)
		cat >"$JQ_MD_COUNT" <<'JQEOF'
[.repositories[].findings[]? | select(.severity == $sev)] | length
JQEOF
		cat >"$JQ_MD_TABLE" <<'JQEOF'
.repositories | to_entries | map(select(.value.findings | length > 0)) | .[] | .key as $repo | .value.findings[] | select(.severity == $sev) | "| [`" + $repo + "`](https://github.com/" + $repo + ") | " + .pattern + " | " + .files + " | " + .description + " |"
JQEOF

		# Group by severity - use results cache for reliable JSON parsing
		for severity in CRITICAL HIGH MEDIUM INFO; do
			# Check if there are findings for this severity
			severity_count=$(jq -r --arg sev "$severity" -f "$JQ_MD_COUNT" "$RESULTS_CACHE" 2>/dev/null)

			if [ "$severity_count" -gt 0 ]; then
				echo "### ${severity} Issues"
				echo ""
				echo "| Repository | Pattern | Files | Description |"
				echo "|------------|---------|-------|-------------|"

				jq -r --arg sev "$severity" -f "$JQ_MD_TABLE" "$RESULTS_CACHE" 2>/dev/null

				echo ""
			fi
		done

		rm -f "$JQ_MD_COUNT" "$JQ_MD_TABLE"
...
"
	...
	if [ "$REPOS_WITH_ISSUES" -gt 0 ]; then
		# Create temp jq file for reliable query execution
		JQ_ISSUE_TABLE=$(mktemp)
		cat >"$JQ_ISSUE_TABLE" <<'JQEOF'
.repositories | to_entries | map(select(.value.findings | length > 0)) | .[] | .key as $repo | .value.findings[] | select(.severity == $sev) | "| [`" + ($repo | split("/") | last) + "`](https://github.com/" + $repo + ") | " + .pattern + " | " + (.files | if length > 80 then .[0:77] + "..." else . end) + " |"
JQEOF

		# Add findings by severity - use results cache for reliable JSON parsing
		for severity in CRITICAL HIGH MEDIUM INFO; do
			severity_count=0
			case $severity in
			CRITICAL) severity_count=$CRITICAL_COUNT ;;
			HIGH) severity_count=$HIGH_COUNT ;;
			MEDIUM) severity_count=$MEDIUM_COUNT ;;
			INFO) severity_count=$INFO_COUNT ;;
			esac

			if [ "$severity_count" -gt 0 ]; then
				ISSUE_BODY+="## ${severity} Issues

| Repository | Pattern | Files |
|------------|---------|-------|
"
				# Generate table rows from results cache
				TABLE_ROWS=$(jq -r --arg sev "$severity" -f "$JQ_ISSUE_TABLE" "$RESULTS_CACHE" 2>/dev/null | head -50)
